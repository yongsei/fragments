export const case8_chapter2_kr = {
  id: 'case8_chapter2',
  title: '챕터 2: 잊혀진 진실',
  story: '엘라라 할머니의 경고 이후 마을의 비밀을 본격적으로 탐색하며, 저널리스트 알렉스와 만나 협력한다. 릴리의 기이한 그림을 통해 과거의 \'잊혀진 의식\'에 대한 중요한 단서를 발견하는 과정.',
  victim: '',
  suspects: [
    {
      id: 'C01',
      name: '알렉스',
      role: '저널리스트',
      description: '이 지역의 실종 사건을 조사하던 젊은 저널리스트. 처음에는 회의적이었으나, 마을의 비정상적인 상황을 점차 받아들이며 여행자와 협력한다. 외부 지식과 논리적 사고를 제공한다.',
      alibi: '',
      motive: ''
    },
    {
      id: 'C02',
      name: '릴리',
      role: '마을 아이',
      description: '마을의 유일한 아이로, 말을 하지 못하고 오직 그림으로만 의사소통한다. 그녀의 그림은 과거의 끔찍한 사건들을 묘사하며 저주의 중요한 단서가 된다.',
      alibi: '',
      motive: ''
    },
    {
      id: 'C03',
      name: '(챕터 2의 추가 인물)',
      role: '마을 주민',
      description: '마을의 식료품점에서 알렉스와 마주친 인물. 그의 표정은 알렉스와 비슷한 당혹감과 피로를 담고 있으며, 마을의 비밀을 어렴풋이 알고 있는 듯하다.',
      alibi: '',
      motive: ''
    },
    {
      id: 'C04',
      name: '(챕터 2의 추가 인물)',
      role: '과거 의식 관련 인물',
      description: '낡은 학교 건물에서 발견되는, 과거의 의식과 관련된 인물에 대한 흔적. 그의 존재는 저주의 기원을 밝히는 데 중요한 역할을 한다.',
      alibi: '',
      motive: ''
    },
    {
      id: 'C05',
      name: '(챕터 2의 추가 인물)',
      role: '의식 주도자',
      description: '릴리의 그림 속에서 반복적으로 등장하는, 의식을 주도했던 인물. 그의 얼굴은 가려져 있지만, 그가 저주의 시작과 깊이 연관되어 있음을 암시한다.',
      alibi: '',
      motive: ''
    },
    {
      id: 'C06',
      name: '(챕터 2의 마지막 인물)',
      role: '핵심 인물',
      description: '\'잊혀진 의식\'의 핵심 인물. 그의 정체는 저주의 본질을 이해하는 데 결정적인 단서가 된다.',
      alibi: '',
      motive: ''
    }
  ],
  evidence: [
    {
      id: 'C13',
      name: '알렉스의 수첩',
      description: '알렉스가 항상 지니고 다니는 낡은 수첩.',
      details: '실종 사건에 대한 기록과 마을의 기이한 현상에 대한 그의 관찰이 담겨 있다.',
      location: 'C01'
    },
    {
      id: 'C14',
      name: '릴리의 그림',
      description: '릴리가 칠판과 바닥에 그린 기괴한 그림들.',
      details: '횃불을 든 사람들이 모여 있는 모습 등 \'잊혀진 의식\'을 묘사하는 듯하다.',
      location: 'C02'
    },
    {
      id: 'C15',
      name: '데자뷰 현상',
      description: '알렉스도 겪었던, 어제와 똑같은 상황이 반복되는 듯한 느낌.',
      details: '이는 마을의 시간 왜곡 현상을 증명하는 중요한 증거이다.',
      location: ''
    },
    {
      id: 'C16',
      name: '실종 사건 기록',
      description: '알렉스가 조사하던 외부인의 실종 사건 기록.',
      details: '이 사건들이 마을의 저주와 연관되어 있음을 시사한다.',
      location: 'C01'
    },
    {
      id: 'C17',
      name: '(챕터 2의 추가 증거)',
      description: '마을 곳곳에서 발견되는, 과거의 의식과 관련된 상징이나 문양.',
      details: '이는 저주의 기원을 밝히는 데 중요한 단서가 된다.',
      location: ''
    },
    {
      id: 'C18',
      name: '(챕터 2의 마지막 증거)',
      description: '\'잊혀진 의식\'의 본질과 그 결과로 발생한 저주에 대한 결정적인 증거.',
      details: '',
      location: ''
    },
    {
      id: 'C19',
      name: '숨겨진 진실',
      description: '식료품점 인물과 마지막 인물의 증언에서 드러나는 새로운 사실.',
      details: '의식의 내용과 참가자들에 대한 더 자세한 정보가 드러난다.',
      location: 'C03'
    },
    {
      id: 'C20',
      name: '연결고리',
      description: '마을의 장소들 사이에서 발견되는 비밀스러운 연결점.',
      details: '의식이 치러졌던 장소들이 어떻게 연결되어 있는지를 보여준다.',
      location: 'C11'
    },
    {
      id: 'C21',
      name: '어둠의 신호',
      description: '과거 의식과 추가 증거 사이에서 발견되는 초자연적 연관성.',
      details: '의식의 힘이 어떻게 저주로 전환되었는지를 드러낸다.',
      location: 'C04'
    },
    {
      id: 'C22',
      name: '깊은 의미',
      description: '실종 사건 기록과 숨겨진 진실이 결합되어 드러나는 저주의 진정한 메커니즘.',
      details: '외부인들이 마을에 끌려오는 이유와 방법을 밝혀낸다.',
      location: 'C19'
    },
    {
      id: 'C23',
      name: '핵심 단서',
      description: '모든 장소와 존재들의 연결이 완성되어 드러나는 결정적 증거.',
      details: '잊혀진 의식의 진정한 목적과 이를 단절시킬 실마리를 제공한다.',
      location: 'C20'
    }
  ],
  locations: [
    {
      id: 'C07',
      name: '식료품점',
      description: '마을의 유일한 식료품점.',
      details: '알렉스와 처음 마주친 장소이며, 낡고 먼지가 쌓여 있어 물품이 거의 없다.'
    },
    {
      id: 'C08',
      name: '낡은 학교 건물',
      description: '마을 외곽에 위치한 폐허가 된 학교.',
      details: '릴리가 웅크리고 앉아 있었으며, 칠판 가득 기괴한 그림들이 그려져 있다.'
    },
    {
      id: 'C09',
      name: '(챕터 2의 추가 장소)',
      description: '마을의 숨겨진 장소 중 하나.',
      details: '이곳에서 과거의 의식과 관련된 유물이나 흔적을 발견할 수 있다.'
    },
    {
      id: 'C10',
      name: '(챕터 2의 추가 장소)',
      description: '릴리의 그림에 자주 등장하는 장소.',
      details: '그곳은 저주의 시작과 깊이 연관된, 마을의 금지된 구역일 수 있다.'
    },
    {
      id: 'C11',
      name: '(챕터 2의 추가 장소)',
      description: '알렉스가 조사하던 실종 사건과 관련된 장소.',
      details: '마을 외부의 사건이 저주와 연결되어 있음을 암시한다.'
    },
    {
      id: 'C12',
      name: '(챕터 2의 마지막 장소)',
      description: '\'잊혀진 의식\'이 거행되었던 핵심 장소에 대한 암시.',
      details: ''
    }
  ],
  connectionRules: [
    {
      id: 'rule_C01_C02_C03',
      cards: ['C01', 'C02'],
      result: '마을의 식료품점에서 알렉스와 마주친 인물에 대한 정보를 발견했습니다.',
      unlock: 'C03'
    },
    {
      id: 'rule_C01_C03_C04',
      cards: ['C01', 'C03'],
      result: '낡은 학교 건물에서 발견되는, 과거의 의식과 관련된 인물에 대한 흔적을 찾았습니다.',
      unlock: 'C04'
    },
    {
      id: 'rule_C02_C04_C05',
      cards: ['C02', 'C04'],
      result: '릴리의 그림 속에서 반복적으로 등장하는, 의식을 주도했던 인물에 대한 단서를 얻었습니다.',
      unlock: 'C05'
    },
    {
      id: 'rule_C03_C05_C06',
      cards: ['C03', 'C05'],
      result: '챕터 2의 마지막 인물에 대한 정보가 해금되었습니다.',
      unlock: 'C06'
    },
    {
      id: 'rule_C07_C08_C09',
      cards: ['C07', 'C08'],
      result: '마을의 숨겨진 장소 중 하나에 대한 정보를 얻었습니다.',
      unlock: 'C09'
    },
    {
      id: 'rule_C07_C09_C10',
      cards: ['C07', 'C09'],
      result: '릴리의 그림에 자주 등장하는 장소에 대한 단서를 발견했습니다.',
      unlock: 'C10'
    },
    {
      id: 'rule_C08_C10_C11',
      cards: ['C08', 'C10'],
      result: '알렉스가 조사하던 실종 사건과 관련된 장소를 알게 되었습니다.',
      unlock: 'C11'
    },
    {
      id: 'rule_C09_C11_C12',
      cards: ['C09', 'C11'],
      result: '챕터 2의 마지막 장소에 대한 정보가 해금되었습니다.',
      unlock: 'C12'
    },
    {
      id: 'rule_C13_C14_C15',
      cards: ['C13', 'C14'],
      result: '데자뷰 현상의 의미를 깨달았습니다.',
      unlock: 'C15'
    },
    {
      id: 'rule_C13_C15_C16',
      cards: ['C13', 'C15'],
      result: '실종 사건 기록에 대한 단서를 얻었습니다.',
      unlock: 'C16'
    },
    {
      id: 'rule_C14_C16_C17',
      cards: ['C14', 'C16'],
      result: '마을 곳곳에서 발견되는, 과거의 의식과 관련된 상징이나 문양을 찾았습니다.',
      unlock: 'C17'
    },
    {
      id: 'rule_C15_C17_C18',
      cards: ['C15', 'C17'],
      result: '챕터 2의 마지막 증거가 해금되었습니다.',
      unlock: 'C18'
    },

    // 추가 연결 규칙들 - 모든 카드가 승리에 필요하도록
    {
      id: 'rule_C03_C06_C19',
      cards: ['C03', 'C06'],
      result: '식료품점 인물과 마지막 인물의 증언이 결합되어 새로운 진실이 드러납니다.',
      unlock: 'C19'
    },
    {
      id: 'rule_C09_C12_C20',
      cards: ['C09', 'C12'],
      result: '숨겨진 장소와 마지막 장소의 연결고리가 발견되었습니다.',
      unlock: 'C20'
    },
    {
      id: 'rule_C04_C10_C21',
      cards: ['C04', 'C10'],
      result: '과거 의식 인물과 그림 속 장소 사이의 숨겨진 연관성이 밝혀졌습니다.',
      unlock: 'C21'
    },
    {
      id: 'rule_C16_C19_C22',
      cards: ['C16', 'C19'],
      result: '실종 사건 기록과 숨겨진 진실이 결합되어 더 깊은 의미를 찾았습니다.',
      unlock: 'C22'
    },
    {
      id: 'rule_C20_C21_C23',
      cards: ['C20', 'C21'],
      result: '장소와 존재들 사이의 연결이 완성되어 핵심 단서가 드러납니다.',
      unlock: 'C23'
    },
    // 최종 승리 조합
    {
      id: 'rule_C22_C23_C18',
      cards: ['C22', 'C23'],
      result: '챕터 2의 모든 진실이 하나로 연결되며, 잊혀진 의식과 저주의 본질이 완전히 드러납니다.',
      unlock: 'C18'
    }
  ],
  fragments: [],
  winCondition: 'C18',
  initialCards: ['C01', 'C02', 'C07', 'C08', 'C13', 'C14']
};